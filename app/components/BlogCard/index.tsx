      ...
      {data.map((blog: iBlog) => (
        <div key={blog.id} className=" flex justify-center items-center">
          <Link key={blog.id} href={{ pathname: `/${blog.id}` }} className="mt-4 lg:mt-10 mb-6 flex flex-col items-start">
            <img
              alt=''
              src={`https://picsum.photos/seed/${blog.id}/2000/400`}
              width={1000}
              height={200}
              className='mx-auto lg:rounded-lg mb-4 drop-shadow-lg'
            />
            <div className='mx-auto max-w-[350px] sm:max-w-[500px] md:max-w-[700px] lg:max-w-[900px]'>
              <h2 className='mx-auto text-white truncate font-extrabold -mt-16 mb-4 text-4xl drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]'>{capitalize(blog.title)}</h2>
              <p className="mx-auto font-medium  mb-4 line-clamp-2 text-gray-600">{capitalize(blog.body)}{blog.body}</p>
